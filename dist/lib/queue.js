...
function createQueueSafely(name, config) {
    try {
...
export function initializeQueues() {
    if (!emailQueue)
        emailQueue = createQueueSafely('email', queueConfig);
    if (!analyticsQueue)
        analyticsQueue = createQueueSafely('analytics', queueConfig);
    if (!inventoryQueue)
        inventoryQueue = createQueueSafely('inventory', queueConfig);
    if (!paymentQueue)
        paymentQueue = createQueueSafely('payment', queueConfig);
    if (!notificationQueue)
        notificationQueue = createQueueSafely('notification', queueConfig);
}
...
function createWorkerSafely(name, processor, config) {
    try {
...
export function initializeWorkers() {
    if (!emailWorker) {
        emailWorker = createWorkerSafely('email', async (job) => {
    ...
    if (!analyticsWorker) {
        analyticsWorker = createWorkerSafely('analytics', async (job) => {
            const { type, data } = job.data;
            switch (type) {
                case 'product_view':
    ...
    if (!inventoryWorker) {
        inventoryWorker = createWorkerSafely('inventory', async (job) => {
            const { type, data } = job.data;
            switch (type) {
    ...
    if (!paymentWorker) {
        paymentWorker = createWorkerSafely('payment', async (job) => {
            const { type, data } = job.data;
            switch (type) {
                case 'process_refund':
    ...
    if (!notificationWorker) {
        notificationWorker = createWorkerSafely('notification', async (job) => {
            const { type, data } = job.data;
            switch (type) {
