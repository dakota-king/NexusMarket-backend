...
export class OrderService {
    ...
    static calculateOrderTotal(cartItems, shippingCost = 0) {
    ...
    // Get vendor orders
    static async getVendorOrders(vendorId, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        const [orders, total] = await Promise.all([
            prisma.order.findMany({
                where: { vendorId },
    ...
    // Get order analytics
    static async getOrderAnalytics(vendorId, startDate, endDate) {
    ...
    static async getOrderStats(vendorId) {
    ...
    static async processFulfillment(orderId, trackingNumber, userId) {
        const order = await this.getOrder(orderId, userId);
    ...
    static async markAsDelivered(orderId, userId) {
        const order = await this.getOrder(orderId, userId);
    ...
    static async handleStatusChange(order, newStatus, _userId) {
