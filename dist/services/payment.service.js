...
export class PaymentService {
    ...
    // Process payment
    static async processPayment(paymentIntentId, orderData) {
        try {
    ...
    static async createVendorOrders(orderData, paymentIntentId) {
        const vendorOrders = await Promise.all(orderData.vendors.map(async (vendorData) => {
            const order = await prisma.order.create({
                data: {
                    ...
                    items: {
                        create: vendorData.items.map((item) => ({
    ...
    static async processVendorPayout(vendorId, amount, orderIds) {
        try {
    ...
    static async getUserSessionData(key) {
