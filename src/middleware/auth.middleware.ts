...
export const requireResourceOwnership = (resourceType: string, getResourceOwnerId: (req: Request) => string | Promise<string>) => {
...
export const requireVendorResourceOwnership = (getVendorId: (req: Request) => string | Promise<string>) => {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      if (!req.user) {
        res.status(401).json({ error: 'Authentication required' })
