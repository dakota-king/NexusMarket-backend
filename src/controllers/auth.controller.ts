...
function determineUserRole(metadata: any): 'CUSTOMER' | 'VENDOR' | 'ADMIN' {
  if (metadata?.role === 'VENDOR') return 'VENDOR'
...
// Delete user address
export async function deleteUserAddress(req: Request, res: Response) {
  try {
    const userId = req.user!.id
    const addressId = req.params.id
